# fastcampus-UX
2019.08.28 ~ 2019.10.16 패스트캠퍼스 UX 디자인 실전 CAMP 과정<br>
12회 진행<br>
강사 : 가재덕<br>
```
2019.08.28 ~ 10.16 (9/11, 9/14, 10/9 휴강)
매주 수요일 19:30 – 22:30
토요일 10:00 – 13:00
주2회, 총 36시간
```


### 2019.08.28 - Day1

## UX 디자인이 뭘까?
UX를 디자인하는 것 -> UX 디자인<br>
디자인 : 주어진 목적을 조형적으로 실체화하는 것<br>
UX : User eXperience 사용자 경험<br>
UX : 시스템 / 제품 / 서비스를 이용하면서 느끼고 생각하게 되는 총체적 경험 (사용자가 느끼는 방식)
<br><strong> UX라는 단어는 정확히 이것이다라고 정의하기 어려움. </strong>
```
Business
Brand
Service
UI
GUI
Contetns


이것은 브랜드,기능,성능,인터페이스,상호작용,사용성,심미성,유희성에 영향을 받게 되지만, (대상과 상호작용)
사용자의 목적,경험,태도,능력,정황,행동등의 영향을 받고(총체적 인간), 
사용전/중/후의 감정,태도,행동,신념,선호,지각,물리,생리적 반응,성취를 포함. (경험
- PXD 이재용 대표
https://story.pxd.co.kr/549

```

- Part of 아이폰 UX

```
Business : 아이폰은 핸드폰
Service : 음악재생 / 앱스토어 / 구글맵
Tech : 멀티터치 / OS X / 센서 / 사파리
UI : full screen / finger touch
Feature : pinch / confernece call
GUI :  cover flow / skeuomorphism(스큐어모피즘)

만약 아이폰이 아이팟터치같은 녀석이었다면? 앱스토어가 없었다면??
이러한 요소들이 UX를 구성.

-> 위 모든 요소들이 아이폰의 UX를 탄생시켰다.

위 모든 요소(Business/Brand/Service/Tech 등) -> point of contact -> USER
사용자를 어떻게 만나야 할까? 사용자 니즈는 무엇일까? 해당 니즈를 해결하기 위해 어떤 기술 어떤 디자인.
또는 어떤 서비스 또는 비지니스를 제공할까?
```

- 수단보다 먼저 있는 목표로서의 UX 디자인

```
예)요금제 탐색과정의 불편함을 해소하고 싶다.
고객 : 비교한뒤에 결정하고 싶다
UI 디자인 : 비교 기능을 요금제 리스트에 노출하고 핵심 내용만 빠르게 비교해 볼 수 있도록 구성하였다.
-> 이거는 리서치를 통한 UI개선 UX디자인이라고 하면 좀 거시기 하다.

고객 : 사용자 95%는 결과적으로 네 개 중 하나의 요금제를 선택한다
Contents/UI 디자인 : 그래서 메인 화면에 `추천 요금제`라는 이름으로 95%의 사용자가 선택하는 네 요금제를 배치했다.
-> 이용 행태의 대한 다방면 대응을 통한 UX 개선(디자인 수준은 아님)

고객 : 조사 결과 사용자는 요금제르 이해하고 선택하는 과정에서 내용을 어려워하고 있다. 따라서, 요금제 선택에 오랜 시간이 걸려 불편이라는 인식이 남게된다.
UX 디자인 : 이해하기 위한 별도의 학습이나 노력 없이도 직관적으로 데잍 요금제를 이해하고 선택하는 경험
Product/Contents/UI 디자인 : 통계 분석을 통해 주로 사용하는 데이터 사용량의 구간을 설정하여 요금제를 설계하고 나머지 요금제를 줄였다.
각 요금제를 쉽게 이해할 수 있또록 S, M, L, XL 라는 이름을 붙여 요금제 리스트 최상단에 노출했다.
-> 위 과정이 사용자 경험을 설계한 UX 디자인이다.
```

- 실제 업무환경에는... UI개선 > 제한 범위의 UX개선 > UX 개선 (제한적으로 수단을 위한 경우로 주로 진행)<br>
그러다 이러한 구분 없이 모두 UX디자인으로 불리는 상황

- 의문기반(왜 그럴까?) / 구조기반(어떤 관계일까?) / 논리기반(말이 되는가?)


---

### 2019.08.31 - Day2

## PROJECT 란?
이 과정에서는 UX 컨설팅 프로젝트를 진행. (전략수립 단계)<br>
사업결정 - 전략수립 - 구축 - 운영/고도화 - 리뉴얼<br>
- CJ 올리브마켓 관련 컨설팅 문서를 보여주심. (cj더마켓)<br>
https://www.cjthemarket.com/pc/main
```
프로젝트는 유일한 물건, 서비스, 결과를 창조하기 위하여 진행되는 한시적인 노력.
무언가를 새로 만들기 위한 한시적인 노력.

시작/종료일이 정해져 있다.
목적 혹은 수행할 작업의 범위가 명확히 명시되어 있다.
예산이 미리 책정되어 있다.
일반적으로 프로젝트가 종료되면 해산하는 임시 조직이 수행한다.(TFT Task Force Team)
해결할 업무가 있다.
일련의 예상되는 효과, 생산품, 결과, 산출물이 정해져 있다.
잘 정의된 역할 할당에 따라 선정된 인력들이 업무를 수행한다.
```

- 프로젝트 주제 : 영화관 서비스(메가박스)
```
1. 예매
2. 다양한 컨텐츠
3. 음식(팝콘, 핫도그, 음료 등)
4. 후기(영화관 또는 상영했던 영화)

왜? 메가박스냐..
- 비교열위 상태
- 독특한 컨셉과 강점
```

- 팀 셋팅
```
1. 총 세팀으로 구성
2. PM(Project Manager) 3명 선출
- 산출물의 퀄리티 체크
- 일정 및 작업량 관리
- TF 구성원 관리
- 실제 작업
- 첫 발표
- 기타 '누가 하지?' 싶은 모든 것
```

### 2019.09.04 - Day3

## TFT 소개 및 발표 (10~15분)
#### RFP 분석/desk research : 비즈니스와 서비스의 이해 / 문제점에 대한 이해
#### Approach 설정. 어떻게 프로젝트를 바라볼 것이나 (목표 / 수단 측면)
```
어떻게 접근 할 것인가??
= 어떤 문제에 집중 할 것인가?

APPROACH 설정을 위해 가장 처음 챙겨야 할 2가지
= WHAT 무엇 & WHY 왜 & HOW 어떻게 어떤걸
무엇을 만들거야? 왜 만들려고? 왜 할려고? 무엇을 이루고자 무슨 목표야?

- 프로젝트 목표/현황
- 서비스 현황
- 시장 환경
- 비지니스 구조

사운즈 한남 예시
WHAT 
- 다른 빌라 단지와 완전히 차별화된 고급 입주처를 만들고 싶다.
WHY
- 획일화 된 고층 오피스텔이 더이상 돈 많은 구매자들에게 매력적이지 않아서
HOW
- 편한 공용 현관문 / 안전한 주차장 / 최첨단 IoT / 도로 가까운 입지.
-> 될까?? WHAT <-> HOW 사이의 인과가 성립하는가?? NO!
   차별화를 만들지 못함.
   경험 : 거주민들이 커뮤니케이션을 즐기며 서로 잘 알고 도움을 주고받을 수 있는 경험
   -> 거주민 사이의 원활한 커뮤니케이션 경험을 고려한 단지 설계(APPROACH)
   
   어떤 문제에 집중 할 것인가?
   1. 거주민 사이의 원활한 커뮤니케이션 경험을 고려한 단지 설계
   2. 편한 공용 현관문 / 안전한 주차장 / 최첨단 IoT / 도로 가까운 입지
   
   요건분석을 해서, 접근방향을 도출해야한다. (RFP Request For Proposal : 제안요청서)
   주어진 자료를 분석하여 고객 경험 측면에서 집중해야 할 목표/범위를 설정하는 과정.
   목적 : 무엇을할건데? < 수단을 통해서, 목적을 달성할수 있는가? > 수단 : 무언가를 할거야
   https://www.similarweb.com/
   http://www.socialmetrics.co.kr/
   https://datalab.naver.com/
   https://trends.google.com/
```


### 2019.09.07 - Day4

```
조사설계 : 아래 4가지를 결정하는 것
목표 : 이번 조사를 통해 무엇을 알고자 하는가?
대상 : 목표 달성을 위해서 누구/어디/무엇을
방법 : 목표 달성을 위해 가장 적합한 방법은 무엇인가?
일정 : 기간은 얼마나 필요한가?

여기서 중요한것은, 목표 설정을 잘 해야한다.(APPROACH 와 같은 관점에서 필요한 조사 항목 설정)

Primary research : AS-IS(있는 그대로, 현재) 서비스와 환경 조사를 통한 조사/분석 틀(research frame) 생성
Secondary research : 사용자 이용 형태 조사를 통한 가설 검증/핵심needs 파악

research frame 설정하는 방법은 다양
Dentsu AISAS 모델을.. 참고하기도 하여 커스텀해서 사용하기도 함.
매킨지 loyalty loop 를 참고하기도 함.

예시)
Behavior : 접근 - 비교/탐색 - 구매 - 이용

이용 단계별 사용흐름
Persona 별 사용흐름
Page 단위 주요 역할
맥락에 따른 분기
이용 단계별 사용 흐름
경로의 이합집산
고정/반복 사용되는 Task
핵심 결과가 되는 이용 페이지

진행 시, 가설을 수립을 하지만.. 모든 UX 프로젝트에서 가설을 수립하지는 않는다.
(경험과 짬밥에 따라.. 아니면 확실한 논리가 있다면..)
가설을 수립하면, 검증을 해야한다.

조사방법 (research method)
(조사 : 사물을 깊이 생각하거나 자세히 조사(자세히 살펴보거나 찾아보는 행동)하여 어떤 이치나 사실을 밝혀내는 것)
사실을 밝혀내는 것
내용을 정확히 알기 위해

정성조사
정량조사

* 데이터를 너무 맹신하지말자.
Page scanning patterns
F 패턴 : 세로 쓰기를 해도 이렇게 나올까??
Z 패턴 : 만약 거꾸로 읽는 국가라면..??

터치하는 패턴
태블릿을 쓰면 컴바인(중복) 영역이 생기겠지만.. 일반 스마트폰이라면..?
참고는 되겠지만.. 너무 맹신하지말자
참고 : https://www.lukew.com/ff/entry.asp?1927 

데이터를 기반으로 UX를 설계시, 아주 조심해야한다.

문제의 근원 : 복잡계(COMPLEX SYSTEM)

현상에는 선형/인과적인 법칙과 규칙이 정해져있음.
넘어져서 아팠다.
사용자는 복잡계 같다.
복잡계 : 다양한 요소의 복잡한 연관관계에 따라 큰 영향을 받음. 
단순계 : 왜 아팠을까? 넘어져서 아프구나.

배가 고파서 아이스크림을 먹었다.
단순계 : 배가고팠기 때문에, 아이스크림을 먹었다.
복잡계 : 밥집이 멀었기 때문에, 배가고파 아이스크림을 먹었다. / 아이스크림밖에 먹을수 없는 섭취시간밖에 없었다.

정성조사 방법론에 대해서 진행.
 
```

- In depth 인터뷰 (심층조사)

```
* 면접 형 대면 인터뷰를 통해 사용자의 니즈를 파악 / 비정형 인터뷰
* 간단한 UT를 병행하거나, 예시를 직접 보여주며 질문을 할 수도 있다.
* 유도적인 질문을 하면 안된다...(답정너가 되면 안되며, 인터뷰어가 눈치를 보고 답변을 하게 된다.)
예) 주로 영화 어디서 보세요? 집앞 CGV요. 왜? CGV 가시나요? 등
```

- Focus Group Interview(FGI) / Discussion(FGD)

```
* 포커스 그룹 인터뷰는 그룹 인터뷰 방식 / 포커스 그룹 디스커션은 인터뷰어들끼리 토론을 한다.
* 그룹 대면 인터뷰 및 참여자 간 토의를 통해 사용자의 니즈를 파악하는 조사 방법.
* 예시를 직접 보여주며, 설명을 할수 있다.
* 비정형 그룹 인터뷰로, 핵심 사용자를 중심으로 사용자들 사이의 의견교환을 통해 예측하지 못했던 인사이트를 발굴 할 수 있다.
* 진행자의 토의 유도/진행 능력이 중요.
```

- Un-Focus Group Interview : n:1 대면 인터뷰 방식의 사용자 조사

```
* 각 분야의 전문가들을 대상으로 한 대면으로 인터뷰
* 주로 서비스디자인 분야에서, 각 전문가들의 견해를 듣거나 인사이트를 발굴하고자 시행
```

- Contextual Interview : 맥락적 인터뷰(Living LAB 이라고 부르기도 함)

```
* 서비스의 특정 상황이나 맥락의 시점에서 이루어지는 대면 인터뷰 방식
* 질문 이외에도 행동 전후 맥락 관찰을 통해 조사 만족도를 제고
* 쉐도잉(요즘은 안함..스토킹 및 개인정보..;; 따라다니면서 행적 기록을 한다 생각하면 될듯) 등의 관찰조사와 병행되어 이루어 지거나, 맥락을 재현하여 조사
예) 영화표를 뽑는 고객에게 잠시 양해를 구하고. -> 이거 아까 이버튼 누르시던데 왜 누르셨어요? 보통 이런식으로 예매표를 뽑나요? 앱으로 이거 동일하게 하실수 있는거 아시나요?
```


+ 정량조사 
- Survey : 비대면 설문조사

```
* 답변 분포로 니즈나 인사이트를 발굴하는 조사방법
* 사전에 설계된 가설, 서비스의 만족도, 이용 상황 및 행태 등을 조사할 수 있다는 장점
* 정량조사인만큼 조사 질문지의 작성과 조사대산의 리쿠르팅, 분석 프레임이 중요.
* 객관식 질문으로 그 외 특이사항/니즈를 발굴가능
* 주관식은 정량 분석에 도움이 되지 않으나, 일부 인사이트 발굴에 기능할 수 있음.
* 한질문안에 주어가 하나만 나와야한다.
* 선택을 강요하면 안됨.
```

- 정리

```
1. RFP 분석
비즈니스와 서비스의 이해/문제점에 대한 이해
어떻게 프로젝트를 바라볼 것인가(목표 / 과제 / 수단)
- APPROACH 설정

2. 리서치 프레임 설정 및 가설설정

3. 리서치 프레임에서 가설을 검증할 수 있는 정성조사를 하자. 정량조사를 하는거보다 정성조사가 더 신뢰적.

4. 조사를 했으면, 이제 분석을 하자. (다음 시간에 분석방법론 진행)


```

---


### 2019.09.18 - Day5

```
분석 : 얽혀 있거나 복잡한 것을 풀어서 개별적인 요소나 성질로 나눔.
- 개념이나 문장을 보다 단순한 개념이나 문장으로 나누어 그 의미를 명료하게함.

복잡한 것 / 모르는 것(research) -> 분석 -> 간단한 것 / 알 수 있는 것 (insight)
폭넓게 조사해온 내용(발상, 확산) -> 분석 -> APPROACH 도출을 위해 좁혀 나가는 것
A가 있었으면, B가 있었으면 -> 분석(왜? A/B를 원하게 됬을까?) -> 제작

UX에서의 분석은 수집된 사용자의 의견을 경험개선의재료로 발굴하는 과정

분석 방법론

Affinity diagram(친화도 다이어그램)
Persona
Journey map
Insight mapping
Laddering

위 방법론이 꼭 분석방법론에만 쓰이는거 아니다.. 어떤것들은 다른곳에서도 사용이 된다.

```


### 2019.09.21 - Day6

```
www.draw.io
Quick And Dirty 

좋은 가설의 특징
- 논쟁의 여지가 있는 가설
- 근거가 있는 가설
- 관련성 있는 가설
- 구체적인 가설
- 행동 지향적인 가설
```

### 2019.09.25 - Day7

```
실습
조사설계 / 질문지 작성
```

### 2019.10.02 - Day8

```
실습
심층인터뷰 및 질문지 정리한걸 기반으로.

Affinity Diagram 
Positioning map 
해보기.
```


### 2019.10.05 - Day9

```
실습
UX 컨셉
UI 패러다임은 늘 변화를 한다..(누구는 이걸 트렌드라고 한다.)
- Skeumorpihism
- Flat
- Material

종료보고서
1. 프로젝트 목표(Approach)
2. 데스크 리서치와 사전 인터뷰를 통한 가설 수립
3. 조사 설계
4. 조사 내용(요약)
5. 분석 내용(요약)
6. 핵심 전략 및 컨셉
7. 와이어프레임
```
